...
- **Teachers**: `GET /api/v1/students/:studentId` (`server/src/api/students/`, `protect` + `isTeacher`) includes `portfolio: { slug, published } | null` so `StudentProfilePage` can link to `/p/:slug` when published and show a **Not live** state when a draft exists but is not public.
